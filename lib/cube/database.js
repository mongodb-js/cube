...
database.config2url = function(config) {
  ...
  var pass = config['mongo-password'];
  var host = config['mongo-host'] || 'localhost';
  var port = config['mongo-port'] || 27017;
  var name = config['mongo-database'] || 'cube';
  var auth = user ? user + ':' + pass + '@' : '';
  return 'mongodb://' + auth + host + ':' + port + '/' + name;
};

database.config2options = function(config) {
  return {
    db: config['mongo-database-options'] || {
        safe: false
    },
    server: config['mongo-server-options'] || {
        auto_reconnect: true
    },
    replSet: {
      read_secondary: true
