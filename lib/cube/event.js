...
exports.getter = function(db) {
  ...
  function getter(request, callback) {
    ...
    var delay = 'delay' in request ? +request.delay : streamDelayDefault;
    var start = new Date(request.start);
    var stop = stream ? new Date(Date.now() - delay) : new Date(request.stop);
    ...
    function query(callback) {
      collection(expression.type).events.find(filter, fields, options, function(error, cursor) {
        ...
        cursor.each(function(error, event) {
          ...
          if (event) {
            callback({
              id: event._id instanceof ObjectID ? undefined : event._id,
              time: event.t,
              data: event.d
            });
          } else {
