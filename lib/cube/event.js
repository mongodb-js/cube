...
var debug = require('debug')('cube:event');
var mongodb = require('mongodb');
var parser = require('../../cube-event-expression');
...
var invalidate = {$set: {i: true}};
var multi = {multi: true};
var metric_options = {capped: true, size: 1e7, autoIndexId: true};
...
exports.putter = function(db) {
  ...
  function putter(request, callback) {
    ...
    if ('id' in request) {
      ...
      debug('event_id', event._id);
    }

    // If this is a known event type, save immediately.
    if (type in knownByType) {
      debug('known event type, save immediately', type, event);
    ...
    if (type in eventsToSaveByType) {
      debug('queue save for', type, event);
    ...
    debug('listCollections', type + '_events');
    var collectionName = type + '_events';
    db.listCollections().toArray(function(error, colls) {
      debug('listCollections', type + '_events returned', error, colls);
      var names = colls.map(function(c) {
        return c.name;
      });
      debug('collection names', names);

      var events = collection(type).events;
      if (names.length) {
        debug('return saveEvents()');
        return saveEvents();
      }
      debug('ensure index', {'t': 1});
