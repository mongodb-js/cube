...
var parser = require('../../cube-metric-expression');
var tiers = require('./tiers');
var types = require('./types');
var reduces = require('./reduces');
var event = require('./event');

var Double = require('bson').Double;

var metric_fields = {v: 1};
var metric_options = {sort: {'_id.t': 1}, batchSize: 1000};
var event_options = {sort: {t: 1}, batchSize: 1000};

// Query for metrics.
exports.getter = function(db) {
  var collection = types(db);
  var queueByName = {};
  var meta = event.putter(db);

  function getter(req, callback) {
    var start = new Date(req.start);
    var stop = new Date(req.stop);
    var id = req.id;
    ...
    var expression;
    try {
      expression = parser.parse(req.expression);
    } catch (e) {
      return callback({error: 'invalid expression'}), -1;
    }

    // Round start and stop to the appropriate time step.
    var tier = tiers[+req.step];
    if (!tier) {
      return callback({error: 'invalid step'}), -1;
    }

    start = tier.floor(start);
    stop = tier.ceil(stop);

    // Compute the req metric!
    measure(expression, start, stop, tier, 'id' in req
  ...
  function unary(expression, start, stop, tier, callback) {
    ...
    var time0 = Date.now();
    var time = start;
    var name = expression.source;
    var queue = queueByName[name];
    var step = tier.key;

    // Compute the expected number of values.
    while (time < stop) ++remaining, time = tier.step(time);

    // If no results were reqed, return immediately.
    if (!remaining) {
