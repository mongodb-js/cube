...
exports.register = function(db, endpoints) {
  ...
  var metric = require('./metric').getter(db);
  var types = require('./types').getter(db);
  ...
  function eventGet(request, response) {
    ...
    if (!(+request.limit <= limitMax)) {
      request.limit = limitMax;
    }
  ...
  function metricGet(request, response) {
    ...
    // If the limit is not specified, or too big, use the maximum limit.
    if (!('step' in request)) {
      request.step = step = 1e4;
    }
    if (!('stop' in request)) {
      request.stop = Math.floor(Date.now() / step) * step;
    }
    if (!('start' in request)) {
      request.start = 0;
    }
    ...
    if ((stop - start) / step > limit) {
      request.start = new Date(stop - step * limit);
    }
    ...
    function callback(d) {
      if (d.time >= stop) {
        response.end(JSON.stringify(data.sort(chronological)));
      } else {
        data.push(d);
      }
    }
